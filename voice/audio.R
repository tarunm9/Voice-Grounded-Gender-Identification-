#packages <- c('tuneR', 'seewave', 'fftw', 'caTools', 'randomForest', 'warbleR', 'mice', 'e1071', 'rpart', 'xgboost', 'rpart-plot')
#for(x in packages){
...
specan3 <- function(X, bp = c(0, 22), wl = 2048, threshold = 5, parallel = 1) {
  ...
  if (parallel > 1) {
    ...
    if (all(Sys.info()[1] == "Windows", requireNamespace("parallelsugar", quietly = TRUE) == TRUE))
      ...
      } else lapp <- function(X, FUN) parallel::mclapply(X, FUN, mc.cores = parallel)
...
x <- as.data.frame(lapp(1:length(start), function(i) {
  ...
  file_name <- sub('\\.wav$', '', sound.files[i])
  snd <- r@left
  print(file_name)
  file_name <- paste(file_name, ".jpeg")
  # demean to remove DC offset
  snd = snd - mean(snd)
  jpeg(file = file_name)
  plot(snd, type = 'l', xlab = 'Samples', ylab = 'Amplitude', col = "#00f7ff")
  dev.off()
...
processFolder <- function(folderName, fileName) {
...
# Load data
feature <- processFolder('static/uploads', myArgs)

# Feature selection
testData <- feature %>% select(duration, meanfreq, sd, median, Q25, Q75, IQR, skew, kurt, sp.ent, sfm, mode, centroid, meanfun, minfun, maxfun, meandom, mindom, maxdom, dfrange, modindx)
